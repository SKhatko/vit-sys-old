...
(function ($) {
    ...
    $.fn.tablesM = function (options) {
        ...
        var settings = $.extend({
            ...
            'tablePicked': function tablePicked(tableId) {}
        }, options);
        ...
        function setMapSizing() {
            if (sectionMap.offset().left > 15) {
                // desktop size section map (with sidebar)
                availableHeight = $(window).height() - sectionMap.offset().top - 20;
                availableWidth = $(window).width() - sectionMap.offset().left - sectionRight.width() - 35;
                ...
                sectionRightPosition = baseW + 35;
            } else {
                // tablet size section map (no sidebar)
                availableHeight = $(window).height() - sectionMap.offset().top - 20;
                availableWidth = $(window).width() - sectionRight.width();
            ...
            sectionRight.css({ left: sectionRightPosition });
        }

        // Insert section map with objects, render it on page
        function renderSectionMap(sectionId, section) {

            var sectionObjects = settings.tablePlanObjects[sectionId];
            section.find('.section-map').html('');

            if (sectionObjects && sectionObjects.length) {
                var sectionObjectsCount = sectionObjects.length - 1;
                for (var i = sectionObjectsCount; i >= 0; i--) {
        ...
        function renderObject(sectionObject, section, editProperties) {
            ...
            var sectionObjectType = sectionObject.type;
            var sectionObjectId = sectionObject.id;
            var sectionId = sectionObject.section_id;
            var sectionObjects = settings.tablePlanObjects[sectionId];
            var activeClass = '';

            if (settings.tableActive == sectionObject.object_num && sectionObjectType == 'table' && settings.tableActive != 0) {
                activeClass = ' active reserved ';
            } else if (settings.tableActive) {
                var activeTablesArray = settings.tableActive.split('+');

                for (var activeTable in activeTablesArray) {
                    if (activeTablesArray[activeTable] == sectionObject.object_num && sectionObjectType == 'table' && settings.tableActive != 0) {
                        activeClass = ' active reserved ';
                    }
                }
            } else if (settings.tablesReserved && settings.tablesReserved.indexOf(sectionObject.object_num) != -1 && sectionObjectType == 'table') {
                activeClass = ' reserved ';
            }

            var sectionObjectHtml = '<div class="tableM-object tableM-object-' + sectionObjectType + activeClass + ' object-' + sectionObjectId + '" title="' + sectionObject.label + '" data-object-id="' + sectionObjectId + '" data-object-type="' + sectionObjectType + '">';

            if (sectionObjectType == 'table') {
                tables.push(sectionObject.object_num);

                sectionObjectHtml += '<span class="tableM-info">';
                sectionObjectHtml += '<strong>#</strong></i><span class="tableM-number" data-object-num="' + sectionObject.object_num + '">' + sectionObject.object_num + '</span>/';
                sectionObjectHtml += '<i class="fa fa-users" aria-hidden="true"></i><span class="tableM-persons">' + sectionObject.persons_num + '</span></span>';
                sectionObjectHtml += '<span class="tableM-label" title="' + sectionObject.label + '">' + sectionObject.label + '</span>';

                if (!settings.tableViewFlag) {
                    sectionObjectHtml += '<span class="tableM-edit-icon hidden-print fa fa-edit" data-object-id="' + sectionObjectId + '"></span>';
                }
            }

            if (!settings.tableViewFlag) {
                sectionObjectHtml += '<span class="tableM-delete-icon hidden-print fa fa-trash-o" data-object-id="' + sectionObjectId + '"></span>';
            }
            sectionObjectHtml += '</div>';

            section.find('.section-map').append(sectionObjectHtml);

            var jObject = $(".object-" + sectionObjectId);
            jObject.css({
                'top': sectionObject.x + '%',
            ...
            jObject.find('.tableM-delete-icon').on('click', function () {
                if (confirm(settings.deleteObjectMsg)) {
                    ...
                    var thisObjectId = thisObject.data('object-id');
                    var objectNumber = thisObject.find('.tableM-number').data('object-num');
                    var index = getIndex(sectionObjects, thisObjectId);
            ...
            jObject.find('.tableM-edit-icon').on('click', function () {
                ...
                var object = $(this);
                var objectNumberInput = editTableModal.find('.object-num-input');
                var personsNumberInput = editTableModal.find('.persons-num-input');
                var oldObjectNumber = $(this).closest('.tableM-object').find('.tableM-number').data('object-num');
                ...
                updateTableButton.on('click', function () {
                    var objectNumber = objectNumberInput.val().replace(/\+/g, "");
                    var personsNumber = +personsNumberInput.val();
                    var personsValidation = true;
                    var tableValidation = true;
                    ...
                    if (personsValidation && tableValidation) {
                        ...
                        var objectId = jObject.data('object-id');
                        var sectionId = jObject.closest('.section-content').data('section-id');
                        var tableIndex = getIndex(settings.tablePlanObjects[sectionId], objectId);
                        var objectProperties = settings.tablePlanObjects[sectionId][tableIndex];
                        var jObjectNumber = jObject.find('.tableM-number');
            ...
            if (settings.tableViewFlag) {
                jObject.on('click', function () {
                    var objectNumber = $(this).find('.tableM-number').data('object-num');
            ...
            if (!settings.tableViewFlag) {
                jObject.draggable({
                    containment: "parent",
                    grid: [5, 5],
                    stop: function stop(e, draggedObject) {
                        setOnBeforeUnload();
                        var index = getIndex(sectionObjects, $(this).data('object-id'));
                        if (index > -1) {
                ...
                if (sectionObjectType == 'pillar' || sectionObjectType == 'plant') {
                    jObject.resizable({
                        ...
                        stop: function stop(e, resizedObject) {
                            var index = getIndex(sectionObjects, $(this).data('object-id'));
                            if (index > -1) {
        ...
        function addEventListeners() {
            ...
            var createObjectButton = $('.object-create-button');
            ...
            $(".save-table-plan").closest('form').submit(function (e) {
                if (settings.tablePlanObjects) {
                    saveObjectsToInput();
                }
                unsetOnBeforeUnload();
            ...
            createObjectButton.click(function (e) {
                ...
                var section = button.closest('.tab-content').find('.section-content.active');
                ...
                if (objectType == 'table') {
                    ...
                    var objectNumberInput = addTableModal.find('.object-num-input');
                    var personsNumberInput = addTableModal.find('.persons-num-input');
                    ...
                    addTableButton.on('click', function () {
                        ...
                        if (personsValidation && tableValidation) {
                            var newObject = new Object(Date.now(), sectionId, objectType, null, null, objectNumber, personsNumber);
                            addObject(newObject, sectionId, section);
                            addTableModal.modal('hide');
                        }
                    });
                } else {
                    var newObject = new Object(Date.now(), sectionId, objectType, null, null);
                    addObject(newObject, sectionId, section);
                }
            });
            ...
            sectionMap.on("drop", function (event) {
                ...
                if (objectType == 'table') {
                    ...
                    addTableButton.on('click', function () {
                        ...
                        if (personsValidation && tableValidation) {
                            var newObject = new Object(Date.now(), sectionId, objectType, percentX, percentY, objectNumber, personsNumber);
                            addTableModal.modal('hide');
                            addObject(newObject, sectionId, section);
                        }
                    });
                } else {
                    var newObject = new Object(Date.now(), sectionId, objectType, percentX, percentY);
                    addObject(newObject, sectionId, section);
                }
            });
        }
        ...
        function editObjectsProperties(object) {
            ...
            var sectionId = object.closest('.section-content').data('section-id');
            var tableIndex = getIndex(settings.tablePlanObjects[sectionId], objectId);
            var objectProperties = settings.tablePlanObjects[sectionId][tableIndex];
            var objectName = $('.object-create-button[data-object-type="' + objectProperties.type + '"]').attr('title');
            ...
            // Disable height input if type of ofject is 'pillar'
            ...
            objectLabelInput.on('input', function () {
                var labelInput = $(this).val();
                object.find('.tableM-label').text(labelInput);
                objectProperties.label = labelInput;
                setOnBeforeUnload();
            });

            objectWidthInput.on('input', function () {
                var widthInput = $(this).val();
            ...
            objectHeightInput.on('input', function () {
                var heightInput = $(this).val();
            ...
            objectBorderRadiusInput.on('input', function () {
                var borderRadiusInput = $(this).val();
...
$(function () {
    $(".js-reception-header__tools-time--button").click(function () {
        var button = $(this);

        if (!button.hasClass('active')) {
            $(".js-reception-header__tools-time--button").removeClass('active');
            button.addClass('active');

            resetRefresher();

            var timeOfDay = getTimeOfDay();
            $("#print-time-of-day").val(timeOfDay);
            $("#lights-shift-input").val(timeOfDay);

            var selectedDate = $(".input-calendar").datepicker('getDate');
            resetDatePicker();
            $(".sidebar-calendar").datepicker('setDate', selectedDate);
        }
    });


    $('.js-reception-header__tools-print').on('click', printReservations);

    $(".js-reception-header__status-online-buttons").click(function () {
        var timeOfDay = getTimeOfDay();

        if (timeOfDay != "all") {
            $("#lights-dialog").modal("show");
            lightsModalOpened = true;
        }
    });
});

function setCurrentDate(hours, minutes, seconds) {
    $(".js-reception-header__tools-date--today").click(function () {

        var currentDate = new Date();

        var checkDate = new Date();

        checkDate.setHours(hours, minutes, seconds);

        $(".js-reception-header__tools-time--button").removeClass('active');
        if (currentDate >= checkDate) {
            $(".js-reception-header__tools-time--button[data-value='night']").addClass('active');
        } else {
            $(".js-reception-header__tools-time--button[data-value='day']").addClass('active');
        }

        resetDatePicker();
        currentDate.setHours(0, 0, 0, 0);
        $('.js-reception-header__tools-date--input').datepicker('setDates', currentDate);
    });
}
